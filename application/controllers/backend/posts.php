...
class Posts extends MY_Backend {

    protected $directory = 'backend/posts/';
    protected $upload_path = 'public/uploads/posts/';
    ...
    public function index() {
        ...
        $joins[] = array(
            'map_select' => 'u.fullname as fullname',
            'map_on' => 'sea_users as u',
            'map_alias' => 'u.id = a.author',
            'map_type' => 'left'
        );
        $order = array(
            'a.created_time' => 'DESC'
    ...
    public function add() {
        ...
        if ($data = $this->input->post()):
            ...
            if ($this->model->search_one('slug', $slug)):
                $errors[] = 'Bài viết này đã tồn tại';
            else:
        ...
        if ($data = $this->input->post()):
            if ($_FILES['file']['name']):
                ...
                if (!$this->upload_file('file', $fileName)):
                    ...
                    $data['post']['thumbnail'] = $this->upload_path . $fileName;
                endif;
            endif;
            if (empty($errors)):
                $slug = $this->util->alias($data['post']['title']);
                $data['post']['slug'] = $slug;
                $data['post']['updated_time'] = date('Y-m-d H:i:s');

                if ($this->model->save($data['post'])):
